% compute regularization energy by examining length and angle...
% differences between the "same" faces in adjacent unit cells

function E = E_regularization(pointsD, face_setsD, same_face_adjs)
    ...
    for i = 1:length(same_face_adjs)        
        ...
        for j = 1:size(same_face_adjs{i},1)
            ...
            angles1 = arrayfun(@(x)atan2_angle(center1(x,:), a1(x,:), b1(x,:)), 1:length(face1));

            center2 = pointsD(face2,:);
            a2 = pointsD(circshift(face2, [0 -1]),:);
            b2 = pointsD(circshift(face2, [0 1]),:);
            angles2 = arrayfun(@(x)atan2_angle(center2(x,:), a2(x,:), b2(x,:)), 1:length(face2));
