...
function [R_ineq, R, J_ineq, J] = all_constraint_residual_and_jacobian(...
    points, edges, edge_pairs, angles, int_rings, bdy_rings, bounds, splines, ...
    ...
    R = [R_edge_pairs(points, edges, edge_pairs); ... % for edge length constraint
         R_dev_rings_generic(points, angles, int_rings); ... % for angle sum constraint
         R_boundary; ... % for boundary matching constraint
         R_boundary_rings(points, angles, bdy_rings); % angle constraint for rectangular boundary
         ]'; 
     
    % assemble Jacobian
    J = [C_edge_pair_rows(points, edges, edge_pairs, fixed); ...
         C_dev_ring_rows_generic(points, angles, int_rings, fixed); ...
         C_boundary; ...
         C_boundary_ring(points, angles, bdy_rings, fixed);
         ]'; 
